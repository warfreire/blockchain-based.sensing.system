...
if [ "$#" -ne 4 ]; then
  echo "Usage: $0 <chaincode container ID> <endorser container ID> <couchdb container ID> <orderer container ID>" >&2
  exit 1
fi
...
while true; do docker stats $1 $2 $3 $4 --no-stream | awk '{print $3 $4}' | tr '\n' ' ' | tr -d 'NAME' >> cpu.stats; echo >> cpu.stats ; done
